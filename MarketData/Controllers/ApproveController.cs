            ...
            try
            {
                ...
                ApproveKeyInDetailViewModel data = new ApproveKeyInDetailViewModel
                {
                    ...
                    BAKeyInDetailList = response.data.Select(c => new BAKeyInDetailData
                    {
                        ID = c.ID,
                        keyInID = c.keyInID,
                        fg = c.fg.HasValue ? string.Format("{0:#,0}", c.fg.Value) : string.Empty,
                        amountSale = c.amountSale.HasValue ? string.Format("{0:#,0}", c.amountSale.Value) : string.Empty,
                        amountSalePreviousYear = c.amountSalePreviousYear.HasValue ? string.Format("{0:#,0}", c.amountSalePreviousYear.Value) : string.Empty,
                        brandID = c.brandID,
                        brandName = c.brandName,
                        ...
                        departmentStoreID = c.departmentStoreID,
                        month = c.month,
                        mu = c.mu.HasValue ? string.Format("{0:#,0}", c.mu.Value) : string.Empty,
                        ot = c.ot.HasValue ? string.Format("{0:#,0}", c.ot.Value) : string.Empty,
                        rank = c.rank,
                        remark = c.remark,
                        sk = c.sk.HasValue ? string.Format("{0:#,0}", c.sk.Value) : string.Empty,
                        week = c.week,
                        wholeSale = c.wholeSale.HasValue ? string.Format("{0:#,0}", c.wholeSale.Value) : string.Empty,
                        year = c.year
                    }).ToList(),
                };
